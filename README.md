...
规则层在已有棋子两格邻域内提取合法候选，计算连五、下一手获胜点、活三等局部特征，按启发式筛选。模型看到的是这些候选的**文字描述和局面摘要**，不是棋盘图像；它不生成自然语言推理过程。

- 普通局面执行模型概率最高的候选点。
- 若可以立即连五，只从获胜候选中选择；否则若对方下一手可以连五，只从防守候选中选择。
- 若规则改变模型首选，界面显示首选、实际落点和介入原因。
- 候选概率是**选择倾向，不是胜率**。棋形标签和规则说明由代码生成。
- 局部特征、六个候选和单步约束无法保证长程棋力。对方同时有多个连五点时可能已无法防守。
...
14 项测试覆盖横竖斜连五及长连、边界、非法/重复落子、终局后落子、活三/冲四、候选合法性，以及“先赢再防”和模型首选被规则接管。另已验证真实模型应手、必防必胜局面，以及浏览器落子、悔棋、重开和连接失败提示。
